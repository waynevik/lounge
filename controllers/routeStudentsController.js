...
const addStudentToRoute = async (req, res) => {
    if(!req?.body?.route_name || !req?.body?.student_id) {
    ...
    if(foundStudentInRoute) return res.status(409).json({ "message": `Student with ID ${req.body.student_id} is already added in route with name ${foundStudentInRoute.route_name}`, "res": "2"});
    ...
    res.status(200).json({"message": "Student has been added to route"});
